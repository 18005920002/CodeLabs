...
public class MsgProducer {
    ...
    public static void main(String[] args) {
        ...
        connectionFactory = new ActiveMQConnectionFactory(MsgProducer.USERNAME, MsgProducer.PASSWORD, MsgProducer.BORKEURL);
        try {
    ...
    public static void sendMessage(Session session, MessageProducer producer) throws Exception {
        for (int i = 0; i < MsgProducer.SENDNUM; i++) {
            TextMessage tm = session.createTextMessage("ActiveMQ SEND " + i);
            System.out.println("发布的消息：" + "ActiveMQ SEND " + i);
            producer.send(tm);
        }
    }
}
